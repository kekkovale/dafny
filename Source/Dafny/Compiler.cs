...
namespace Microsoft.Dafny {
  public class Compiler
  {
    ...
    public Compiler(bool quickyCompile) {
        this.quickyCompile = quickyCompile;
    }
...
//        }
    ...
    private void CompileMethod(ClassDecl c, int indent, Method m, TextWriter wr) {
      ...
      if (quickyCompile) {
        CheckPreconditions(m, wr);
        CreateCounterExampleString(m, wr);
      }
      if (m.Body == null) {
        Error("Method {0} has no body", wr, m.FullName);
      } else {
    ...
    private void CheckPreconditions(Method m, TextWriter wr) {
      foreach (var precondition in m.Req) {
        wr.Write("if(!(");
        TrExpr(precondition.E, wr, false);
        wr.WriteLine(")) {qChecker.PreconditionFailed();return;}");
      }
    }
      ...
      if (pat.Var != null) {
        ...
        for (int i = 0; i < pat.Arguments.Count; i++) {
          ...
          if (formal.IsGhost) {
            ...
            TrCasePatternOpt(arg, null,
              string.Format("(({0})({1})._D).@{2}", DtCtorName(ctor, ((DatatypeValue) pat.Expr).InferredTypeArgs, wr),
                tmp_name, FormalName(formal, k)), indent, wr, inLetExprBody);
            k++;
          }
        }
      }
